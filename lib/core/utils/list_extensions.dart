  ...
  List reshape(List<int> newShape) {
    ...
    if (newShape.length == 2) {
      ...
      List<List<dynamic>> reshaped = List.generate(
        rows,
        (_) => List.filled(cols, null, growable: false),
        growable: false,
      );
    ...
    if (newShape.length == 3) {
      ...
      List<List<List<dynamic>>> reshaped = List.generate(
        ...
        (_) => List.generate(
          d2,
          (_) => List.filled(d3, null, growable: false),
          growable: false,
        ),
        growable: false,
      );
    ...
    if (newShape.length == 4) {
      ...
      List<List<List<List<dynamic>>>> reshaped = List.generate(
        ...
        (_) => List.generate(
          ...
          (_) => List.generate(
            d3,
            (_) => List.filled(d4, null, growable: false),
            growable: false,
          ),
          growable: false,
        ),
      ...
      for (int i = 0; i < length; i++) {
        reshaped[i ~/ stride1][(i % stride1) ~/ stride2][(i % stride2) ~/
                stride3][i % stride3] =
            this[i];
      }
      return reshaped;
